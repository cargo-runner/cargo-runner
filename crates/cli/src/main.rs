    ...
    if !args.is_empty() && args[0].ends_with("cargo-runner") {
        ...
        if args.len() > 1 && args[1] == "runner" {
            ...
            match runner.command {
                Commands::Analyze { filepath, verbose, config } => analyze_command(&filepath, verbose, config),
...
fn print_formatted_analysis(runner: &mut cargo_runner_core::CargoRunner, filepath: &str, line: Option<usize>, show_config: bool) -> Result<()> {
    ...
    if runnables.is_empty() {
        ...
        for (i, runnable) in runnables.iter().enumerate() {
            ...
            if matches!(runnable.kind, cargo_runner_core::RunnableKind::DocTest { .. }) {
            ...
            if let Some(line_num) = line {
                ...
                debug!("Runnable '{}' contains line {}? {}", runnable.label, line_num + 1, contains);
            }
...
fn parse_cargo_command(args: &[String]) -> (Option<String>, Option<String>, Vec<String>, Vec<String>) {
    ...
    while i < args.len() {
        ...
        if after_separator {
            ...
            if matches!(arg.as_str(), "--bin" | "--example" | "--test" | "--bench" | "--features") {
                extra_args.push(arg.clone());
...
fn init_command(cwd: Option<&str>, force: bool) -> Result<()> {
    ...
    println!("🚀 Initializing cargo-runner in: {}", project_root.display());
    ...
    if !root_config_path.exists() || force {
        ...
        fs::write(&root_config_path, root_config)
            .with_context(|| format!("Failed to write root config to {}", root_config_path.display()))?;
        info!("Created root config: {}", root_config_path.display());
        created += 1;
    } else {
        info!("Skipping existing root config: {}", root_config_path.display());
        skipped += 1;
    }
    ...
    if skipped > 0 {
        println!("   • Skipped {} existing configs (use --force to overwrite)", skipped);
    }
...
fn print_config_details(_runner: &cargo_runner_core::CargoRunner, filepath: &str) -> Result<()> {
    ...
    if let Some(command) = &merged_config.command {
        println!("      • command: {}", command);
    }
    if let Some(subcommand) = &merged_config.subcommand {
    ...
    if let Some(linked_projects) = &merged_config.linked_projects {
        println!("      • linked_projects: {} projects", linked_projects.len());
    }
    if !merged_config.overrides.is_empty() {
        println!("      • overrides: {} configured", merged_config.overrides.len());
    }
