...
impl<'a> CommandBuilder<'a> {
    ...
    pub fn build(self) -> Result<CargoCommand> {
        ...
        let builder: Box<dyn CommandBuilderImpl> = match (&self.runnable.kind, file_type) {
            ...
            (RunnableKind::DocTest { .. }, FileType::CargoProject) => {
                Box::new(DocTestCommandBuilder)
            }
            (RunnableKind::Test { .. }, FileType::CargoProject) => Box::new(TestCommandBuilder),
            (RunnableKind::ModuleTests { .. }, FileType::CargoProject) => {
                Box::new(ModuleTestCommandBuilder)
            }
            (RunnableKind::Benchmark { .. }, FileType::CargoProject) => {
                Box::new(BenchmarkCommandBuilder)
            }
        };
...
trait ConfigAccess {
    ...
    fn get_test_framework<'a>(
        ...
        config: &'a Config,
        file_type: FileType,
    ) -> Option<&'a crate::config::TestFramework> {
        match file_type {
...
trait CommandBuilderImpl: ConfigAccess {
    ...
    fn apply_common_config(
        ...
        let extra_env = match file_type {
            ...
            FileType::SingleFileScript => config
                .single_file_script
                .as_ref()
    ...
    fn apply_features(
        ...
        if let Some(cargo_config) = &config.cargo {
            collected_features =
                Features::merge(collected_features.as_ref(), cargo_config.features.as_ref());
        }
        ...
        if let Some(override_config) = self.get_override(runnable, config, file_type) {
            if override_config.force_replace_features.unwrap_or(false) {
                ...
                // Merge features
                collected_features = Features::merge(
                    collected_features.as_ref(),
                    override_config.features.as_ref(),
                );
            }
        }
...
impl CommandBuilderImpl for BinaryCommandBuilder {
    fn build(
        ...
        debug!(
            "BinaryCommandBuilder: binary_framework = {:?}",
            self.get_binary_framework(config, file_type)
        );
        debug!(
            "BinaryCommandBuilder: runnable.module_path = '{}', package = {:?}",
            runnable.module_path,
            config.cargo.as_ref().and_then(|c| c.package.as_ref())
        );
        ...
        if let Some(binary_framework) = self.get_binary_framework(config, file_type) {
            ...
            let is_custom_command = binary_framework
                .command
                .as_deref()
                .map_or(false, |cmd| cmd != "cargo");
        ...
        let is_cargo_command =
            if let Some(binary_framework) = self.get_binary_framework(config, file_type) {
                binary_framework.command.as_deref() == Some("cargo")
                    || binary_framework.command.is_none()
            } else {
                true
            };

        // Determine if we're using standard run subcommand
        let is_run_subcommand =
            if let Some(binary_framework) = self.get_binary_framework(config, file_type) {
                binary_framework.subcommand.as_deref() == Some("run")
                    || binary_framework.subcommand.is_none()
            } else {
                true
            };
    ...
    fn build(
        ...
        match &runnable.kind {
            RunnableKind::Test { test_name, .. } => {
                ...
                let mut command = CargoCommand::new_rustc(args).with_test_filter(test_name.clone());
...
impl CommandBuilderImpl for SingleFileScriptBuilder {
    fn build(
        ...
        match &runnable.kind {
            ...
            _ => Err(crate::Error::ParseError(
                "Not a single file script or test".to_string(),
            )),
        }
    }
