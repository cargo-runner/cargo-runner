...
impl CargoRunner {
    ...
    pub fn get_best_runnable_at_line(
        ...
        debug!(
            "get_best_runnable_at_line: found {} runnables",
            runnables.len()
        );
        ...
        if file_path.file_name() == Some(std::ffi::OsStr::new("main.rs")) {
            ...
            runnables.push(Runnable {
                ...
                scope: Scope {
                    ...
                    kind: ScopeKind::File(crate::types::FileScope::Bin {
    ...
    fn build_bazel_command_for_runnable(
        &self,
        runnable: &Runnable,
    ...
    pub fn get_fallback_command(&mut self, file_path: &Path) -> Result<Option<CargoCommand>> {
        ...
        debug!(
            "get_fallback_command: current build_system={:?}, project_root={:?}",
            self.build_system, self.project_root
        );
        ...
        debug!(
            "get_fallback_command: before ensure_project_root, build_system={:?}",
            self.build_system
        );
        self.ensure_project_root(&resolved_path)?;
        debug!(
            "get_fallback_command: after ensure_project_root, build_system={:?}",
            self.build_system
        );
            ...
            if let Some(ref project_root) = self.project_root {
                ...
                let synthetic_runnable = crate::types::Runnable {
                    ...
                    kind: if file_name == "main.rs"
                        || resolved_path.to_string_lossy().contains("src/bin")
                    {
                ...
                debug!(
                    "get_fallback_command: Returning Bazel command with overrides: {:?}",
                    command
                );
        ...
        debug!(
            "get_fallback_command: cargo.binary_framework={:?}",
            self.config
                .cargo
                .as_ref()
    ...
    fn ensure_project_root(&mut self, file_path: &Path) -> Result<()> {
        debug!(
            "ensure_project_root: file_path={:?}, current build_system={:?}",
            file_path, self.build_system
        );
        ...
        if self.project_root.is_some() && self.build_system.is_none() {
            if let Some(ref project_root) = self.project_root {
                ...
                debug!(
                    "Detected build system from PROJECT_ROOT: {:?}",
                    self.build_system
                );
            }
            ...
            if found_root.is_none() {
                if let Ok(cwd) = std::env::current_dir() {
                    ...
                    debug!(
                        "Using CWD as project root, build system: {:?}",
                        self.build_system
                    );
                }
            }
        ...
        debug!(
            "ensure_project_root: final build_system={:?}, project_root={:?}",
            self.build_system, self.project_root
        );
    ...
    fn reload_config_for_path(&mut self, file_path: &Path) -> Result<()> {
        ...
        debug!(
            "Reloaded config for path {:?}: cargo.binary_framework={:?}",
            file_path,
            self.config
                .cargo
                .as_ref()
    ...
    fn resolve_file_path_internal(&self, file_path: &Path) -> Result<(PathBuf, Option<PathBuf>)> {
        ...
        debug!(
            "Current config has linked_projects: {:?}",
            self.config
                .cargo
                .as_ref()
                .and_then(|c| c.linked_projects.as_ref())
                .is_some()
        );
        ...
        if let Some(cargo) = &self.config.cargo {
            if let Some(linked_projects) = &cargo.linked_projects {
                ...
                for linked_project in linked_projects {
                    ...
                    let project_dir = if linked_path.file_name()
                        == Some(std::ffi::OsStr::new("rust-project.json"))
                    {
        ...
        if let Some(project_root) = &self.project_root {
            ...
            if candidate.exists() {
                ...
                return Ok((
                    candidate,
                    project_dir.or_else(|| Some(project_root.clone())),
                ));
            }
    ...
    pub fn get_command_at_position_with_dir(
        ...
        debug!(
            "After ensure_project_root: cargo.binary_framework={:?}",
            self.config
                .cargo
                .as_ref()
                .and_then(|c| c.binary_framework.as_ref())
    ...
    pub fn get_package_name_str(&self, file_path: &Path) -> Result<String> {
        ...
        if let Some(cargo_toml) = ModuleResolver::find_cargo_toml(file_path) {
            ...
            cargo_toml
                ...
                .map(|s| s.to_string())
                .ok_or_else(|| {
                    crate::error::Error::ParseError("Could not determine package name".to_string())
                })
        } else {
