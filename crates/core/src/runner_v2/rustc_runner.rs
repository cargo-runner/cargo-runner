...
impl RustcRunner {
    ...
    /// Build a rustc command for a single-file script
    fn build_rustc_command(&self, file_path: &Path, runnable: &Runnable) -> CargoCommand {
        let mut args = vec![];
        
        // Basic rustc invocation
        args.push(file_path.to_string_lossy().to_string());
        args.push("-o".to_string());
        
        // Output binary name (remove .rs extension)
        let output_name = file_path
            .file_stem()
            .and_then(|s| s.to_str())
            .unwrap_or("rust_binary");
        ...
        // For tests, add test flag
        if matches!(runnable.kind, RunnableKind::Test { .. }) {
        ...
        CargoCommand {
            ...
            args,
            env: vec![],
            working_dir: file_path.parent().map(|p| p.to_string_lossy().to_string()),
            test_filter: None,
        }
    }
}
...
impl CommandRunner for RustcRunner {
            ...
            let file_runnable = Runnable {
                scope: crate::types::Scope {
                    ...
                    kind: crate::types::ScopeKind::File(crate::types::FileScope::Standalone { 
                        name: file_path.file_stem().and_then(|s| s.to_str()).map(|s| s.to_string()) 
                    }),
                    name: Some(file_path.to_string_lossy().to_string()),
                },
                kind: RunnableKind::Binary {
                    bin_name: file_path.file_stem()
                        .and_then(|s| s.to_str())
                        .map(|s| s.to_string()),
                },
    ...
    fn build_command(
        ...
        Ok(self.build_rustc_command(&runnable.file_path, runnable))
    }
    
    fn validate_command(&self, command: &Self::Command) -> Result<()> {
        // Rustc-specific validation
        if command.args.is_empty() {
            return Err(crate::error::Error::Other("Rustc command has no arguments".to_string()));
        }
        
        // Ensure it's a rustc command
        match &command.command_type {
            CommandType::Rustc => Ok(()),
            _ => Err(crate::error::Error::Other("Expected Rustc command type".to_string())),
        }
    }
