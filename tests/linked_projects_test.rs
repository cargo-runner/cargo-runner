...
"#,
    ...
    let cmd = runner
        .get_command_at_position_with_dir(&root_dir.join("project-a/src/main.rs"), Some(8))
        .unwrap();
    ...
    let cmd = runner
        .get_command_at_position_with_dir(&root_dir.join("project-b/src/lib.rs"), Some(4))
        .unwrap();
...
"#,
    ...
    // Create the file that the test will check
    fs::write(
        src_dir.join("non_existent.rs"),
        r#"
fn main() {
    println!("Test");
}
"#,
    ...
    // Test with the file in src directory
    let cmd = runner
        .get_command_at_position_with_dir(&src_dir.join("non_existent.rs"), None)
        .unwrap();
